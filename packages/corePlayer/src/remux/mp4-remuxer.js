    ...
    */
    ...
    _remuxAudio(audioTrack, force) {
        const concatBuffers = (buffers) => {
          ...
          function processAudioData(audioArray) {
            
            let processedArray = [];
            let lastPts = audioArray[0].pts; // Initialize to a very small number
        
            audioArray.forEach(dataPoint => {
                if (dataPoint.pts > lastPts) {
                    processedArray.push(dataPoint);
                    lastPts = dataPoint.pts;
                }else{
                    if(logDebug===true){
                        console.warn("mp4-remux","触发音頻pts 矫正")
        ...
        if (this._audioMeta == null) {
            ...
            // _audioMeta 不存在可能是 独立音频轨道 或者是 pcm 数据（其实也是独立音频track）
    ...
    _remuxVideo(videoTrack, force) {
        ...
        let videoDtsBase=this._videoDtsBase  // 在非  splitAVBuffers 模式下 应该是 _dtsBase
        ...
        //let firstSampleOriginalDts = samples[0].dts - this._dtsBase;
        let firstSampleOriginalDts = samples[0].dts - videoDtsBase;
        ...
        for (let i = 0; i < samples.length; i++) {
            let sample = samples[i];
           // let originalDts = sample.dts - this._dtsBase;
           let originalDts = sample.dts - videoDtsBase;//splitAVBuffers 模式
            ...
            if (i !== samples.length - 1) {
                // let nextDts = samples[i + 1].dts - this._dtsBase - dtsCorrection;
                let nextDts = samples[i + 1].dts - videoDtsBase - dtsCorrection;
                sampleDuration = nextDts - dts;
            } else {  // the last sample
                if (lastSample != null) {  // use stashed sample's dts to calculate sample duration
                    //let nextDts = lastSample.dts - this._dtsBase - dtsCorrection;
                    let nextDts = lastSample.dts - videoDtsBase - dtsCorrection;
                    sampleDuration = nextDts - dts;
                } else if (mp4Samples.length >= 1) {  // use second last sample duration
                    sampleDuration = mp4Samples[mp4Samples.length - 1].duration;
                } else {  // the only one sample, use reference sample duration
        ...
        if(logDebug===true){
            console.info("mp4-sample1",info)
            console.info("mp4-sample2",mp4Samples)
        }

        if(logDebug===true){

            if(mp4Samples.filter((v)=>{return v.duration<0}).length>0){
                debugger
