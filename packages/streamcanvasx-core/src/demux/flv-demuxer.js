...
class FLVDemuxer {
    ...
    parseChunks(chunk, byteStart) {
        ...
        while (offset < chunk.byteLength) {
           ...
           // let timestamp = ts0 | (ts1 << 8) | (ts2 << 16) | (ts3 << 24);
            let timestamp=((ts0 | (ts1 << 8) | (ts2 << 16)) + (ts3 << 24)) >>> 0;

            let streamId = v.getUint32(7, !le) & 0x00FFFFFF;
            //StreamID 是 FLV tag header 中的一个字段，正常為0 沒有什麽用但是如果不是0 則可能是一個異常tag 忽略
            if (streamId !== 0) {
            ...
            switch (tagType) {
                case 8:  // Audio
                    ...
                    if(logDebug===true){
                        console.log(this.TAG,`audioTag timestamp_${timestamp/1000}秒    TAGPosition${byteStart + offset}`);
                    }
                    break;
                case 9:  // Video
                    if(logDebug===true){
                        console.log(this.TAG,`videoTag timestamp_${timestamp/1000}秒    TAGPosition${byteStart + offset}`);
                    }
        ...
        if (this._isInitialMetadataDispatched()) {
            if (this._dispatch && (this._audioTrack.length || this._videoTrack.length)) {
                ...
                // 每次接受新chunk ，解析完 chunk里面的 音视频tag ，   dispatch parsed frames to consumer 
    ...
    _parseAudioData(arrayBuffer, dataOffset, dataSize, tagTimestamp) {
        ...
        if (soundFormat !== 2 && soundFormat !== 10&&soundFormat!=7&&soundFormat!=8) {  // MP3 or AAC
        ...
        if (soundFormat === 10) {  // AAC
            ...
            } else if (aacData.packetType === 1) {  //  AAC原始帧數據 AAC raw frame data
                let dts = this._timestampBase + tagTimestamp;
                let aacSample = {unit: aacData.data, length: aacData.data.byteLength, dts: dts, pts: dts};

                //音频track 过滤 ，抛弃非递增的AAC音频帧 , 可以統一放到Mp4 remux 邏輯中處理
                
                // try{
                //     if(track.samples&&track.samples.length>0){
                //         if(track.samples[track.samples.length-1].pts>aacSample.pts){
                //             // debugger
                //             return false
                //         }
                //     }
    ...
    _parseVideoData(arrayBuffer, dataOffset, dataSize, tagTimestamp, tagPosition) {
        ...
        let frameType = (spec & 0b01110000) >>> 4;  //frameType 1 则是关键帧
    ...
    _parseAVCVideoPacket(arrayBuffer, dataOffset, dataSize, tagTimestamp, tagPosition, frameType) {
        ...
        if(logDebug===true){
            ...
            console.log(this.TAG, `videoPacket H264  tagTimestamp:${tagTimestamp/1000}秒 cts:${cts}  frameType:${frameType}`);
    ...
    _parseHEVCVideoPacket(arrayBuffer, dataOffset, dataSize, tagTimestamp, tagPosition, frameType) {
        ...
        if(logDebug===true){
            ...
            console.log(this.TAG, `videoPacket H265  tagTimestamp:${tagTimestamp/1000}秒 cts:${cts}  frameType:${frameType}`);
