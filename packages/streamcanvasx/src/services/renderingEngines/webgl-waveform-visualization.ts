    ...
    initgl() {
      ...
      updateVertBuffer() {
        ...
        for (let i = 0; i < this.totalWaveforms; i++) {
         ...
         // let data = this.translatePointe(pcmData, heightScale, verticalOffset);
          ...
          let array = _.takeRight(this.vertBuffer[i], 2 * (this.bufferLength - this.updateLength));
          this.vertBuffer[i] = _.concat(array, data);
      ...
      convertPCMToVertices(pcmData: Float32Array, heightScale: number, verticalOffset: number) {
        ...
        for (let index = 0; index < sampleCount; index++) {
          ...
          vertices.push(y);
          // 添加沿x轴对称的点
          vertices.push(-y + 2 * verticalOffset);
        }
