export default function WebGLYUVRenderer(gl: WebGLRenderingContext, openWebglAlignment: boolean): { render: (w: number, h: number, y: Uint8Array, u: Uint8Array, v: Uint8Array) => void; destroy: () => void } {
    ...
    const vertexPosBuffer = gl.createBuffer() as WebGLBuffer;
    ...
    const texturePosBuffer = gl.createBuffer() as WebGLBuffer;
    ...
    let yTextureRef = _initTexture('ySampler', 0);
    let uTextureRef = _initTexture('uSampler', 1);
    let vTextureRef = _initTexture('vSampler', 2);
