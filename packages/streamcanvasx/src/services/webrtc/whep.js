...
params = {};
		...
		if (links.hasOwnProperty('urn:ietf:params:whip:core:layer')) this.layerUrl = new URL(links['urn:ietf:params:whip:core:layer'][0].url, url);
	...
	async trickle() {
		...
		let { endOfcandidates } = this;
		const { restartIce } = this;
		...
		if (fetched.status == 200) {
			...
			const { remoteDescription } = this.pc;
	...
	async selectLayer(layer) {
		if (!this.layerUrl) throw new Error('WHIP resource does not support layer selection');
